...
public class SistemaElectoral {
    ...
    public void Votar(int candidato) {
        ...
        if (buscar(cc) == false) {//se valida que la persona no haya votado 
            ...
            agregarLista(p);
            candidatos[candidato] += 1; //se cuenta el voto 
    ...
    //se busca la cc en la lista para saber si la persona ya votó
    ...
    public void resultados() {
        ...
        res="\n\nEL CANDIDATO GANADOR FUE EL NUMERO " + (ganador + 1);
        //validar empate
        for (int i = 0; i < candidatos.length; i++) {
